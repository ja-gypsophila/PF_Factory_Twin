...
- **생산량 차트** — 시간별·일별 생산/불량. 조업시간(08~19시 KST) 동안 실제 생산분을 시간대별로 실집계
...
│   └ 단일 WS 연결을         │                        │  · 순차 라인 결합(병목 지배)│
│     Context로 전 페이지 공유│                        │  · 조업 08~19시(KST) 실집계│
│                           │                        │  · 자정 리셋 + 7일 롤오버  │
│                           │                        │  · /health (keep-alive)   │
│  / (대시보드)             │                        └─────────────────────────┘
│  /machine/:id (상세)      │
  ...
  "machines": [
    {
      ...
      "targets": { "oee": 0.75, "availability": 0.88, "dailyCount": 5000 },
      "hourProduction": [
        /* 8시~현재 시간대별 실제 생산 누적 [{ hour, prod }] */
      ],
    },
    // … 용접기 / 사출성형기 / 검사기
  ],
...
- **병목 공정 (Theory of Constraints)** — 뒤 공정은 앞 공정이 만든 재공품만큼만 생산할 수 있어, 가장 느린 설비(사출기)가 라인 전체 산출을 실시간으로 지배 → `isBottleneck`으로 표시
...
- **조업시간 기반 실집계 + 지난 시간대 자동 채움** — 08~19시(KST)에만 지표를 수집하고, 제품이 완성되는 순간 시간대 버킷에 실집계. 서버가 조업 중간에 재시작되면 지나간 시간대만 같은 생산 공식으로 채워 넣어 차트가 비어 보이지 않게 함.
- **실시간 라인 결합** — 뒤 공정은 앞 공정의 재공품(WIP)만큼만 생산 가능하도록 결합해, 병목(사출기)이 라인 산출을 실제로 제한. "하류 공정이 상류보다 많이 생산"하는 물리적 모순을 차단.
- **자정(KST) 일일 리셋** — 서버(UTC)에서 매 틱 KST 날짜를 비교해 자정에 금일 지표를 리셋하고 어제치를 7일 이력에 롤오버. 자정에 서버가 잠들어 있어도 깨어난 직후 따라잡음.
...
> ℹ️ 조업 시간(08:00~19:59 KST) 외에는 설비가 대기(IDLE)로 표시되고 지표 수집이 중단됩니다. 지나간 시간대 차트는 서버 기동 시 자동으로 채워집니다.
